...
async function setupScenes() {
  ...
  if(store.ignoredScenes) {
    console.log("Found ignoredScenes in sync storage.. moving to local storage")
    await chrome.storage.local.set({"ignoredScenes": store.ignoredScenes});
    console.log("Deleting ignoredScenes from sync storage");
  ...
  console.log("Store Size: " + megaBytes.toFixed(2) + " MB");
    ...
    chrome.runtime.sendMessage({msg: "queryLocalStash", query: { query, variables: { id } } }, results => {
      ...
      display.hover(
        function() {
          ...
          let menu = $("<div>", { class: "dropdown-menu", style: "display: block; background: white; position: absolute; top: 5px; left: 25px; padding: 8px; border-radius: 4px; width: 110px; font-size: 14px; font-weight: normal"});
